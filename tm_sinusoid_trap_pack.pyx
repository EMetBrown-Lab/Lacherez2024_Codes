...
cdef dtype_t phi_prime_1(dtype_t q, dtype_t alpha, dtype_t l) : # Potential' 1
...
@cython.cdivision(True)
cdef dtype_t phi_1(dtype_t q, dtype_t alpha, dtype_t l) :    # Potential 1
...
def _Peq_projection1D_q1(q1, alpha, l) : # Peq particle
    projection = np.exp(-(phi_1(q1, alpha, l)))
    return projection

def _Peq_projection1D_q2(q2, kappa) : # Peq wall
    projection = np.exp(-(phi_2(q2, kappa)))
    return projection

def Peq_projection1D_q1(q1, alpha, l): # Normalised 
    P = np.array([_Peq_projection1D_q1(q1i, alpha, l) for q1i in q1])
...
cdef qi_full_arrays(dtype_t[:] eta_1, dtype_t[:] eta_2,
    ...
    cdef dtype_t[:] q1_theo = np.linspace(-l, l, N) # Vague range of q1, start from equilibrium purposes
    cdef dtype_t[:] q2_theo = np.linspace(-.1, .1, N) # Vague range of q2

    inverse_cdf_q1 = sample_sin(q1_theo, Peq_projection1D_q1, alpha, l)
    inverse_cdf_q2 = sample_optic(q2_theo, Peq_projection1D_q2, kappa)
    
    q1[0] = inverse_cdf_q1(np.random.uniform(0.000000001, (1-0.000000001)))
    q2[0] = inverse_cdf_q2(np.random.uniform(0.000000001, (1-0.000000001)))

    cdef dtype_t _phi_prime_1
    cdef dtype_t _phi_prime_2
    cdef int i
    for i in range(1, N):  
        _phi_prime_1 = phi_prime_1(q1[i-1], alpha, l) 
...
def sample_sin(q_sample, Peq_projection1D_q, alpha, l) : 
    y_q = Peq_projection1D_q1(q_sample, alpha, l) 
    cdf_y_q = np.cumsum (y_q)
    cdf_y_q = cdf_y_q/cdf_y_q.max() 
    inverse_cdf_q = interp1d(cdf_y_q,q_sample) 
    return inverse_cdf_q